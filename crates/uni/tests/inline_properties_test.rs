...
async fn test_inline_property_simple() -> Result<()> {
    ...
    let result = db.query("MATCH (n:Person {name: 'Bob'}) RETURN n.name, n.age").await?;

    println!("Result length: {}", result.len());
    if result.len() > 0 {
        println!("First row: name={}, age={}",
...
async fn test_inline_property_multiple() -> Result<()> {
    ...
    // Create schema
    db.execute("CREATE LABEL Product (name STRING, price INT, category STRING)").await?;

    // Create test data
    db.execute("CREATE (:Product {name: 'Laptop', price: 1000, category: 'Electronics'})").await?;
    db.execute("CREATE (:Product {name: 'Mouse', price: 25, category: 'Electronics'})").await?;
    db.execute("CREATE (:Product {name: 'Desk', price: 300, category: 'Furniture'})").await?;

    // Test multiple inline properties
    let result = db.query("MATCH (p:Product {category: 'Electronics'}) RETURN p.name ORDER BY p.name").await?;
