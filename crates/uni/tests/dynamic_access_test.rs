...
async fn test_dynamic_map_access() {
    ...
    let result = db.query("WITH 'a' AS key RETURN {a: 42}[key] AS val").await.unwrap();
    ...
    let result = db.query("MATCH (n:Person) RETURN n.name AS val").await.unwrap();
    assert_eq!(result.rows[0].value("val").unwrap(), &Value::String("Alice".to_string()));
...
async fn test_keys_function_node() {
    ...
    db.query("CREATE (n:Person {name: 'Alice', age: 30})").await.unwrap();
