...
async fn test_return_property_access() -> Result<()> {
    ...
    let result = db.query("MATCH (n:Person) RETURN n.name, n.age ORDER BY n.name").await?;
...
async fn test_return_graph_introspection_functions() -> Result<()> {
    ...
    let result = db.query("MATCH (n:Person) RETURN labels(n) AS node_labels").await?;
    println!("labels() result: {:?}", result.rows()[0].value("node_labels"));

    // Test type() function
    let result = db.query("MATCH ()-[r:WORKS_AT]->() RETURN type(r) AS rel_type").await?;
    println!("type() result: {:?}", result.rows()[0].value("rel_type"));

    // Test properties() function
    let result = db.query("MATCH (n:Person) RETURN properties(n) AS props").await?;
