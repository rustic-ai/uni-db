...
fn test_trig_functions() {
    // ASIN
    let res = eval_scalar_function("ASIN", &[json!(0.0)]).unwrap();
    assert_eq!(res.as_f64().unwrap(), 0.0);

    let res = eval_scalar_function("ASIN", &[json!(1.0)]).unwrap();
    assert!((res.as_f64().unwrap() - std::f64::consts::FRAC_PI_2).abs() < 1e-10);

    // ACOS
    let res = eval_scalar_function("ACOS", &[json!(1.0)]).unwrap();
    assert_eq!(res.as_f64().unwrap(), 0.0);

    let res = eval_scalar_function("ACOS", &[json!(0.0)]).unwrap();
    assert!((res.as_f64().unwrap() - std::f64::consts::FRAC_PI_2).abs() < 1e-10);

    // ATAN
    let res = eval_scalar_function("ATAN", &[json!(0.0)]).unwrap();
    assert_eq!(res.as_f64().unwrap(), 0.0);

    let res = eval_scalar_function("ATAN", &[json!(1.0)]).unwrap();
    assert!((res.as_f64().unwrap() - std::f64::consts::FRAC_PI_4).abs() < 1e-10);

    // ATAN2
    let res = eval_scalar_function("ATAN2", &[json!(1.0), json!(1.0)]).unwrap();
    assert!((res.as_f64().unwrap() - std::f64::consts::FRAC_PI_4).abs() < 1e-10);

    // DEGREES
    let res = eval_scalar_function("DEGREES", &[json!(std::f64::consts::PI)]).unwrap();
    assert!((res.as_f64().unwrap() - 180.0).abs() < 1e-10);

    // RADIANS
    let res = eval_scalar_function("RADIANS", &[json!(180.0)]).unwrap();
    assert!((res.as_f64().unwrap() - std::f64::consts::PI).abs() < 1e-10);

    // HAVERSIN
    let res = eval_scalar_function("HAVERSIN", &[json!(0.0)]).unwrap();
    assert_eq!(res.as_f64().unwrap(), 0.0);

    // haversin(pi) = (1 - cos(pi)) / 2 = (1 - (-1)) / 2 = 1
    let res = eval_scalar_function("HAVERSIN", &[json!(std::f64::consts::PI)]).unwrap();
    assert!((res.as_f64().unwrap() - 1.0).abs() < 1e-10);
}
